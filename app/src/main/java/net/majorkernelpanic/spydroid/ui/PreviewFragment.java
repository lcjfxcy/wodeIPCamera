...
public class PreviewFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		if (((SpydroidActivity)getActivity()).device == ((SpydroidActivity)getActivity()).TABLET) {
			...
			/*mZoomBar=(SeekBar)mZoomBar.findViewById(R.id.mZoomBar);
            ...
            mZoomBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
				...
				public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
					EventBus.getDefault().post(new Progress(progress));
			...
			/*mCamera=SessionBuilder.getInstance().getcamera();

			if (mZoomBar==null)
			{
				Log.v("aaaaaaaaaa","frag is null");}
			mZoomBar.setMax(mCamera.getParameters().getMaxZoom());
			mZoomBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
				@Override
				public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
